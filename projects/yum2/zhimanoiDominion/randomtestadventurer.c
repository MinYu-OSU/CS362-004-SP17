...
void testAdventurer(int *passed, int *failed)
{
    ...
    //play card
    adventurerCard(drawnTreasure, &testG, whoseTurn, z, temphand, cardDrawn);

    // TEST 1: Check that handCount has 2 additional cards
    if ((G.handCount[whoseTurn] + drawnTreasure) == testG.handCount[whoseTurn]) {
        printf ("Test 2 - Check players hand count increment: Expected Value: %i || Returned Value: %i === PASSED ===\n", (G.handCount[whoseTurn]+2), testG.handCount[whoseTurn]);
    }
    else {
